...
/**
 * The 'v-swap-fade' directive is a conditional displaying utility.
 *
 * It's similar to 'v-show-fade' but with the difference that it wait some time (a delay)
 * before showing the element.
 * This is useful when you want to hide and element and show another (i.e. to swap them),
 * using animations (this can't be done using the standard 'v-show-fade' directive because
 * it will show both elements at the same time while performing the animations).
 *
 * This directive has one modifier: 'fast' for increase the speed of the swap.
 */
Vue.directive("swapFade", {
  bind: function(el, binding) {
    // Add base CSS classes.
    addClassIfNeed(el, FADE_BASE);
    if (binding.modifiers && binding.modifiers.fast) {
  ...
  update(el, binding) {
    ...
    if (shouldHide) {
      ...
      // Wait a delay before starting the show animation.
      el.dataset.delayedIfTimeout = setTimeout(() => {
        // Show element but faded.
        removeClassIfNeed(el, FADE_HIDDEN);
        addClassIfNeed(el, FADE_FADDED);
